...
The paper's classical table also has one PICCOLO row and four SM4 rows. They are
intentionally not duplicated here because this publication repository admits
only AES and Ascon. The quantum table also has a proof-only AES MixColumns
retained-copy row without a same-boundary result/predecessor IR pair; it is
omitted under this repository's admission rule. The admitted AES and Ascon rows
below otherwise reproduce the final PDF table values.
...
| Ascon inverse S-box; exactly 6 ANDs; complement free | [13 XORs, depth 17](ir/baselines/classical/ascon_inverse_sbox_6and_13xor_depth17.json) | [12 XORs, depth 19](ir/results/classical/ascon_inverse_sbox_6and_12xor_depth19.json) | 1 fewer XOR; full depth increases by 2; the [32-XOR source comparator](ir/baselines/classical/ascon_inverse_sbox_mcoptimal_binary_6and_32xor.json) is retained; no 11-XOR lower bound is claimed |
...
| Specialized Ascon S-box `S(x0,x1,x2 xor 1,x3,x4)`; 5-qubit logical NCT | [17 gates, 7 Toffolis, logical/source-weighted depths 11/53](ir/baselines/quantum/ascon_sbox_round_constant_x2_specialized_17gate_nct.json) | [16 gates, exact 7 Toffolis, depths 11/53](ir/results/quantum/ascon_sbox_round_constant_x2_specialized_16gate_nct.json) | 1 fewer `X` and total gate |
| Ascon retained-copy linear layer; 640-wire `(x,0^320) -> (L(x),x)`; CNOT; depth at most 3, with depth-at-most-4 bound also reported | [960 CNOTs, depth 3](ir/baselines/quantum/ascon_linear_oh_et_al_copy_960cnot_depth3.json) | Exact 960 CNOTs at depth at most 3; 480-960 CNOTs at depth at most 4 in the paper | The complete 960-CNOT construction is replayed here; the structural lower-bound proofs are intentionally not included |
